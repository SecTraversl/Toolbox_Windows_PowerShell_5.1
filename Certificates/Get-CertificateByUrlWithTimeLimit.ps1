<#
.SYNOPSIS
  The "Get-CertificateByUrlWithTimeLimit" function is an improvement on the 'Get-CertificateByUrl' function in that it includes a built-in timer parameter, which by default, waits for a website response for 5 seconds before moving on.  The returned objects include certificate information for each website resolved from the URLs given as arguments to the "-URL" parameter.
...
.OUTPUTS
.NOTES
  Name:  Get-CertificateByUrlWithTimeLimit.ps1
  Author:  Travis Logue
  Version History:  1.2 | 2021-08-26 | Created Pipeline support and embedded dependencies
  Dependencies: 
  Notes:
...
function Get-CertificateByUrlWithTimeLimit {
  [CmdletBinding()]
  [Alias('CertByUrlWithTimeLimit')]
  param (
  ...
  begin {
    ...
    } # End 'New-JobWithTimeLimit' block
  ...
  process {
    ...
    foreach ($item in $URL) {
      New-JobWithTimeLimit -ScriptBlock ${function:Get-CertificateByUrl} -ArgumentList $item -TimeLimit 5 -Verbose
    }
