<#
.SYNOPSIS
  The "Get-CertificateByUrl" function has been replaced by the "Get-CertificateByUrlWithTimeLimit" function.  The reason for the replacement is that 'Get-CertificateByUrlWithTimeLimit' has a built-in timer that expires after a certain number of seconds (without this built-in timer the connection could hang for half an hour or longer because an expected response was not received).  This 'Get-CertificateByUrl' function along with the original notes and references are kept here as documentation.
...
.OUTPUTS
.NOTES
  Name:  Get-CertificateByUrl.ps1
  Author:  Travis Logue
  Version History:  1.1 | 2021-05-07 | Combining two separate functions into one
  Dependencies: 
  Notes:
    - This is where I got the basic code for this idea: http://stackoverflow.com/questions/22233702/how-to-download-the-ssl-certificate-from-a-website-using-powershell/22236908
    - This was helpful in understanding how to update the "User-Agent" string: https://stackoverflow.com/questions/33659663/how-to-set-user-agent-with-system-net-webrequest-in-c-sharp
    - This was helpful when searching for meaningful dialogue on specifying the "default properties" that are displayed, while still keeping the rest of the properties accessible (Select-Object within this function removed the rest of the properties so they were not accessible after the function ran): https://community.idera.com/database-tools/powershell/ask_the_experts/f/learn_powershell_from_don_jones-24/13909/script-to-return-capture-all-properties-but-not-display-all-properties-to-screen
    - Kirk Munro had the syntax necessary for the "default properties" that get displayed, I found out about this from the "idera.com" forum mentioned above: http://poshoholic.com/2008/07/05/essential-powershell-define-default-properties-for-custom-objects/
    - Boe Prox also has a reference for how to create a "Default Display Properties Set" so only select properties are displayed by default (though all properties are still part of the object): https://learn-powershell.net/2013/08/03/quick-hits-set-the-default-property-display-in-powershell-on-custom-objects/
    
#>

function Get-CertificateByUrl {
  [CmdletBinding()]
  [Alias('CertByURL')]
  param (
    [Parameter(ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, HelpMessage = "Reference the URL, e.g. 'https://www.outlook.com' .")]
    [string[]]
    $URL
  )
  ...
  process {
    ...
    foreach ($item in $URL) {
      ...
      $UserAgentList = @(
        'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36' #One of the most popular user agent strings - 2020-12-23 - Security Onion - event_type:bro_http - (Add a filter) useragent:"exists"
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36' #Another popular user agent - 2020-12-23 - Security Onion - event_type:bro_http - (Add a filter) useragent:"exists"
      )
  
      $UserAgent = Get-Random $UserAgentList
      ...
      $prop = [ordered]@{
        WebResponseUri   = $Response.ResponseUri
        WebCharacterSet  = $Response.CharacterSet
        WebServer        = $Response.Server
        WebLastModified  = $Response.LastModfied
        WebStatusCode    = $Response.StatusCode
        WebVersion       = $Response.ProtocolVersion
        WebMethod        = $Response.Method
        IsFromCache      = $Response.IsFromCache
        CertThumbprint   = $cert.GetCertHashString()
        CertValidFrom    = $cert.GetEffectiveDateString()
        CertValidUntil   = $cert.GetExpirationDateString()
        CertSerialNumber = $cert.GetSerialNumberString()
        CertFormat       = $cert.GetFormat()
        CertSubject      = $cert.Subject
        #CertPublicKeyString = $cert.GetPublicKeyString()
        #CertRawCertData = $cert.GetRawCertDataString()
      }
      $obj = New-Object -TypeName psobject -Property $prop
  
      $defaultProperties = @( 'CertThumbprint', 'CertValidFrom', 'CertValidUntil', 'CertSerialNumber', 'WebResponseUri' )
