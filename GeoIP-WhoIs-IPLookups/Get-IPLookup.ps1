<#
.SYNOPSIS
  The "Get-IPLookup" function combines a DNS PTR lookup and a query of the 'http://whois.arin.net/' API in order to retrieve DNS Hostname and WhoIs information about the specified IP Address.

.DESCRIPTION
.EXAMPLE
  PS C:\> Get-IPLookup -IPAddress 217.170.205.14, 219.91.110.131
...
.EXAMPLE
  PS C:\> IPLookup 40.107.220.52 -Verbose
  VERBOSE: Starting Get-IPLookup
  ...
  VERBOSE: 52.220.107.40.in-addr.arpa


  IPAddress           : 40.107.220.52
  PtrRecord           : mail-co1nam11on2052.outbound.protection.outlook.com
  ...
  Here we run the function using its built-in alias of "IPLookup".  We also run the function with the "-Verbose" Common Parameter, which allows us to see information about what the code is doing as it steps along.

.INPUTS
.OUTPUTS
.NOTES
  Name:  Get-IPLookup.ps1
  Author:  Travis Logue
