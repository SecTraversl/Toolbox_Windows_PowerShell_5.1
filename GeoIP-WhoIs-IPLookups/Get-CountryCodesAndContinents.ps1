<#
.SYNOPSIS
  The "Get-CountryCodesAndContinents" function displays information containing a Country's name, two letter country code, three letter country code, the country's number, continent, and continent code.  

.DESCRIPTION
.EXAMPLE
  PS C:\> Get-CountryCodesAndContinents -CountryCode 'ge','am','kz','ru','tr','az','cy','um' | ft

  ContinentName ContinentCode CountryName                          CountryCode ThreeLetterCode CountryNumber
  ------------- ------------- -----------                          ----------- --------------- -------------
  ...
  Here we run the function with a list of the two-letter Country Codes.  We then pipe that to "Format-Table" or "ft" where we see the Continent Code and Country Name for each of those Country Codes we referenced.

.EXAMPLE
  PS C:\> $test = Get-CountryCodesAndContinents -FullCountryName
  PS C:\>
  PS C:\> $test | select -f 30 | ft
  ...
  Here we specified the "-FullCountryName" Switch Parameter when running the function.  The output renders the full, verbose name of each country.

.INPUTS
.OUTPUTS
.NOTES
  Name: Get-CountryCodesAndContinents.ps1
  Author: Travis Logue
  Version History: 1.0 | 2021-01-20 | Initial Version
  Dependencies: 
  Notes:
  - This was where I pulled a copy of the data set: https://datahub.io/JohnSnowLabs/country-and-continent-codes-list
        # - I saved a copy of that data set as: Country-Code_And_Continent_LookupTable.csv
        #  There were some duplicate entries, mapping 8 countries to two different continents
        #  GE - Georgia - I removed it from Europe and kept it in Asia
        #  AM - Armenia - I removed it from Europe and kept it in Asia
        #  KZ - Kazakhstan - I removed it from Europe and kept it in Asia
        #  RU - Russia - I removed it from Europe and kept it in Asia
        #  TR - Turkey - I removed it from Europe and kept it in Asia
        #  AZ - Azerbaijan - I removed it from Europe and kept it in Asia
        #  CY - Cyprus - I removed it from Europe and kept it in Asia
        #  UM - United States Minor Outlying Islands - I removed it from Oceania and kept it in North America


  - In order to embed the data from the .csv into this function I did the following:
        #  We saved the data table as a .csv using Excel.  Then we imported it into PowerShell, and pasted the output into this function
        #  $x = ipcsv .\Country-Code_And_Continent_LookupTable.csv | ConvertTo-Csv -NTI
        #  $x | scb


  .
#>
function Get-CountryCodesAndContinents {
  [CmdletBinding()]
  [Alias('CountryCodes')]
  param (
    [Parameter(HelpMessage='Reference the Country Code the code you want to use for the lookup.')]
    [string[]]
    $CountryCode,
    [Parameter(HelpMessage='This Switch Parameter is used to display the full version of the Country Name.')]
    [switch]
    $FullCountryName
  )
  ...
  process {
    ...
    # Here is how we created this output quickly:
    #  We saved the table as a .csv using Excel.  Then we imported it into PowerShell, and pasted the output into this function
    #  $x = ipcsv .\Country-Code_And_Continent_LookupTable.csv | ConvertTo-Csv -NTI
    #  $x | scb
    


    $LookupTableArray = [System.Collections.ArrayList]@(
      @("Asia","AS","Afghanistan, Islamic Republic of","AF","AFG","4"),
      @("Europe","EU","Albania, Republic of","AL","ALB","8"),
      @("Antarctica","AN","Antarctica (the territory South of 60 deg S)","AQ","ATA","10"),
      @("Africa","AF","Algeria, People's Democratic Republic of","DZ","DZA","12"),
    ...
    #$Header = 'ContinentName','ContinentCode','CountryName','CountryCode','ThreeLetterCode','CountryNumber'



    if ($FullCountryName) {
      $ObjectForm = foreach ($Line in $LookupTableArray) {
        $prop = [ordered]@{
          ContinentName = $Line[0]
          ContinentCode = $Line[1]
          CountryName = $Line[2]
          CountryCode = $Line[3]
          ThreeLetterCode = $Line[4]
          CountryNumber = $Line[5]
        }
      ...
      $ObjectForm = foreach ($Line in $LookupTableArray) {
        $prop = [ordered]@{
          ContinentName = $Line[0]
          ContinentCode = $Line[1]
          CountryName = $Line[2] -replace ",.*" -replace "\(.*"
          CountryCode = $Line[3]
          ThreeLetterCode = $Line[4]
          CountryNumber = $Line[5]
        }
    ...
    if ($CountryCode) {
      foreach ($Code in $CountryCode) {
        $ReturnTheFollowing = $ObjectForm | ? {$_.CountryCode -eq $Code}
        Write-Output $ReturnTheFollowing
      }
    }
    else {
