...
.EXAMPLE
  PS C:\> *ConversE <tab>
  PS C:\> Get-tsharkConversEth

  

  Here we are demonstrating a fast way to invoke this function.  Simply typing "*ConversE" and then pressing the Tab key should result in the full function name.

.EXAMPLE
  PS C:\> $Convers_NoFilter = Get-tsharkConversEth -Pcap .\myTest.pcap
...
.EXAMPLE
  PS C:\> $Convers_PublicIP = Get-tsharkConversEth -Pcap .\myTest.pcap -FilterSet PublicIP
...
.EXAMPLE
  PS C:\> $Convers_PvtIpOnly = Get-tsharkConversEth -Pcap .\myTest.pcap -FilterSet PrivateIPOnly
...
.EXAMPLE
  PS C:\> $Convers_Multicast = Get-tsharkConversEth -Pcap .\myTest.pcap -FilterSet Multicast
...
function Get-tsharkConversEth {
  ...
  param (
    ...
    [string]
    $FilterSet,
    [Parameter(HelpMessage='This Switch Parameter is used to turn off MAC Vendor Name resolution.')]
    [switch]
    $NoMacResolution
  )
  ...
  process {
    ...
    if ($DisplayFilter) {
      if ($NoMacResolution) {
        $Convers = tshark.exe -n -r $Pcap -q -z conv,eth,$DisplayFilter
      }
      else {
        $Convers = tshark.exe -r $Pcap -q -z conv,eth,$DisplayFilter
      }
    }
    else {
      if ($NoMacResolution) {
        $Convers = tshark.exe -n -r $Pcap -q -z conv,eth
        $Header = $Convers | Select-Object -Skip 1 -First 1
        $Header = $Header.Insert(0,"`n") + "`n"
      }
      else {
        $Convers = tshark.exe -r $Pcap -q -z conv,eth
        $Header = $Convers | Select-Object -Skip 1 -First 1
        $Header = $Header.Insert(0,"`n") + "`n"
      }
    }
    ...
    $ObjectForm = $TrimTopAndBottom -replace "\s+","`t" | ConvertFrom-Csv -Delimiter "`t" -Header 'Eth1','Dir','Eth2','<-Frames','<-Bytes','Frames->','Bytes->','AllFrames','AllBytes','RelativeStart','Duration'
