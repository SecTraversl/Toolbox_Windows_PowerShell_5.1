...
function Get-SchTasks {
  ...
  process {
    ...
    $AllResults = foreach ($Computer in $ComputerName) {
      $exe = 'schtasks.exe'
      [array]$param = @()
      $param += '/Query','/FO:LIST','/V','/S',"$Computer"   # Final result should look like: "schtasks.exe /query /FO:LIST /V /S $Computer"
      $Results = & $exe $param | Where-Object {$_}   # We are using "Where-Object {$_} to remove the blank lines from the output of the .exe"
